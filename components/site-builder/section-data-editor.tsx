...
export const SectionDataEditor = ({ clientId, onNext, initialData }: SectionDataEditorProps) => {
  ...
  const uploadImage = async (file: File, fieldName: string): Promise<string> => {
    ...
    try {
      // Crear un archivo con nombre específico
  ...
  // Función para generar nombres específicos de archivos
  const getSpecificFileName = (fieldName: string, originalName: string): string => {
    ...
    switch (fieldName) {
      ...
      case 'about-image':
        return `about.${extension}`;
      case 'services-background':
        return `services-bg.${extension}`;
      case 'contact-background':
        return `contact-bg.${extension}`;
      case 'testimonials-background':
        return `testimonials-bg.${extension}`;
      case 'new-testimonial-image':
        return `testimonial.${extension}`;
      default:
        if (fieldName.startsWith('product-') && fieldName.endsWith('-image')) {
          const productIndex = fieldName.match(/product-(\d+)-image/)?.[1] || '1';
          return `product-${productIndex}.${extension}`;
        }
        if (fieldName.startsWith('testimonial-') && fieldName.endsWith('-image')) {
          const testimonialIndex = fieldName.match(/testimonial-(\d+)-image/)?.[1] || '1';
          return `testimonial-${testimonialIndex}.${extension}`;
        }
        ...
        <div className="mt-2">
          {preview ? (
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-gray-400 transition-colors">
              ...
              >
                <span className="font-medium vendes-text-primary hover:vendes-text-primary/80">
                  Subir imagen
                </span>
          ...
          {uploading && (
            <div className="mt-2 flex items-center text-sm vendes-text-primary">
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 vendes-border-primary mr-2"></div>
              Subiendo imagen...
            </div>
          )}
        </div>
            ...
            <Card className="p-4 border-dashed">
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <Plus className="h-5 w-5 vendes-text-primary" />
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 p-4 bg-[#551BB3] rounded-lg vendes-border-primary">
